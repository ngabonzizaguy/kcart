...
export function OrderTracking({ 
  ...
  // FEATURE FLAGS: Check what features are enabled
  ...
  // Convert order prop to OrderData format, fallback to mock if not provided
  const convertOrderToOrderData = (orderProp: any): OrderData => {
    if (!orderProp) return mockOrderData;
    
    // Helper function to safely convert order time
    const convertOrderTime = (createdAt: any): string => {
      try {
        if (!createdAt) return '2:30 PM';
        const date = new Date(createdAt);
        if (isNaN(date.getTime())) return '2:30 PM';
        ...
        console.warn('Error converting order time:', error);
        return '2:30 PM';
      }
    };

    // Helper function to safely convert item names
    const convertItemName = (itemName: any) => {
      if (typeof itemName === 'string') {
        return { en: itemName, rw: itemName };
      }
      if (itemName && typeof itemName === 'object' && itemName.en) {
        return itemName;
      }
      // Fallback for any other case
      return { en: 'Food Item', rw: 'Ibiryo' };
    };

    try {
      return {
        id: String(orderProp.id || 'DG-2025-001'),
        status: orderProp.status || 'preparing',
        estimatedDelivery: orderProp.estimatedDelivery || '25-35 mins',
        originalETA: orderProp.originalETA || '2:55 PM',
        etaUpdateReason: orderProp.etaUpdateReason,
        vendorName: String(orderProp.vendorName || 'Restaurant'),
        vendorPhone: String(orderProp.vendorPhone || '+250 788 123 456'),
        items: Array.isArray(orderProp.items) ? orderProp.items.map((item: any) => ({
          name: convertItemName(item?.name),
          quantity: Number(item?.quantity) || 1,
          price: Number(item?.price) || 0
        })) : mockOrderData.items,
        total: Number(orderProp.total) || 0,
        orderTime: convertOrderTime(orderProp.createdAt),
        address: String(orderProp.deliveryAddress || 'Kigali, Rwanda'),
        paymentMethod: orderProp.paymentMethod === 'card' || orderProp.paymentMethod === 'cash' 
          ? orderProp.paymentMethod 
          : 'momo',
        transactionRef: String(orderProp.transactionRef || `TXN-${Date.now()}`),
        deliveryMethod: orderProp.deliveryMethod === 'drone' ? 'drone' : 'standard'
      };
    } catch (error) {
  ...
  const [orderData, setOrderData] = useState(() => convertOrderToOrderData(order));

  // Update order data when order prop changes
  useEffect(() => {
    if (order) {
      setOrderData(convertOrderToOrderData(order));
    }
  }, [order]);
      ...
      <div className="h-screen bg-gradient-to-br from-orange-50 via-amber-50 to-orange-100 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide">
          <div className="px-6 py-6 pb-32">
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-3xl p-4 border border-white/20 mb-6">
              {orderData.status === 'cancelled' ? (
                ...
                <>
                  ...
                  {/* Delivery Method Indicator - Feature Flag Controlled */}
                  {featureFlags['delivery.droneDelivery'] && orderData.deliveryMethod === 'drone' && (
            ...
            {/* Review Prompt Modal - Shows when order is delivered (Feature Flag Controlled) */}
            {featureFlags['social.reviewsAndRatings'] && (
              <ReviewPrompt
                ...
                onRatingOnly={handleQuickRating}
                onClose={() => setShowReviewPrompt(false)}
              />
            )}

            {/* Reviews Modal (Feature Flag Controlled) */}
            {featureFlags['social.reviewsAndRatings'] && (
              <ReviewsModal 
              ...
              <div className="space-y-4 mb-6">
                ...
                {(orderData.status === 'preparing' || orderData.status === 'out-for-delivery') && (
                  <div className="space-y-3">
                    {/* Enhanced Tracking Modal Button - Feature Flag Controlled */}
                    {featureFlags['delivery.enhancedTracking'] && onOpenEnhancedTracking && (
                      <Button 
                    ...
                    {/* Drone Tracking Button - Feature Flag Controlled */}
                    {featureFlags['delivery.droneDelivery'] && orderData.deliveryMethod === 'drone' && onOpenDroneTracking && (
                      <Button 
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-3xl p-6 border border-white/20 mb-6">
              ...
              <div className="border-t border-gray-200 pt-4 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    {React.createElement(getPaymentMethodIcon(orderData.paymentMethod), { className: "w-4 h-4 text-gray-600" })}
                    <span className="text-gray-700 text-sm font-medium">{getPaymentMethodName(orderData.paymentMethod)[language]}</span>
                  </div>
                </div>
                
                <div className="flex items-center justify-between">
                  <span className="text-gray-600 text-sm">{content[language].transactionRef}</span>
                  <span className="text-gray-700 text-sm font-mono">{orderData.transactionRef}</span>
                </div>
                
                <div className="flex items-center justify-between">
                  <span className="text-gray-600 text-sm">{content[language].deliverTo}</span>
                  <span className="text-gray-700 text-sm">{orderData.address}</span>
                </div>
              </div>
            </div>
          </div>
