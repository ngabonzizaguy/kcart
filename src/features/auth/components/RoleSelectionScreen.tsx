...
interface RoleSelectionScreenProps {
  language: 'en' | 'rw';
  onRoleSelect: (role: 'customer' | 'vendor' | 'admin') => void;
  onBack: () => void;
}
  ...
  const content = {
    en: {
      ...
      admin: {
        ...
        subtitle: 'Manage the entire DeliGo platform',
        benefits: [
          'Oversee all users and vendors',
    ...
    rw: {
      ...
      admin: {
        title: 'Umuyobozi w\'Urubuga',
        subtitle: 'Gucunga urubuga rwose rwa DeliGo',
        benefits: [
          'Gukurikirana abakoresha n\'abacuruzi bose',
          'Gukurikirana isesengura ry\'urubuga',
          'Gucunga igenamiterere y\'urubuga',
          'Kwita ku mutekano w\'urubuga'
        ],
        cta: 'Injira mu Buyobozi'
      },
      back: 'Subira'
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-amber-50 to-orange-100 flex flex-col pb-safe">
      ...
      <div className="pt-safe px-6 py-6">
        <div className="flex items-center justify-between mb-8 mt-4">
          <button
            onClick={onBack}
            className="text-gray-600 hover:text-gray-800 transition-colors text-base font-medium rounded-xl px-4 py-2 hover:bg-white/50"
          >
      ...
      <div className="flex-1 px-6 space-y-6">
        {/* Customer Card */}
        <Card 
          className="bg-white/80 backdrop-blur-sm border border-white/20 cursor-pointer transform transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] rounded-3xl"
        ...
        >
          <CardContent className="p-6">
            <div className="flex items-start gap-4 mb-4">
              ...
              <div className="flex-1">
                <h3 className="text-lg font-medium text-gray-800 mb-1">
                  {content[language].admin.title}
                </h3>
                <p className="text-sm text-gray-600 mb-4">
                  {content[language].admin.subtitle}
                </p>
                
                {/* Benefits */}
                <div className="space-y-2 mb-4">
                  {content[language].admin.benefits.map((benefit, index) => {
                    const icons = [Users, BarChart, Settings, Shield];
                    const IconComponent = icons[index % icons.length];
            ...
            <Button className="w-full bg-purple-500 hover:bg-purple-600 text-white rounded-xl h-12 text-base font-medium">
              {content[language].admin.cta}
            </Button>
          </CardContent>
        </Card>
      </div>
